...
library(calendR)
calendR(year = 2021,
        title = "Crimerate xxxx - yyyy", # Title
        title.size = 30,                 # Title size
        start = "M",
        weeknames = c("", "", "", "",    # Week names
                      "", "",""), 
        special.days = days,
...
ggplot(df, aes(x = days, y = y, fill = value)) +
...
g <- ggplot(df, aes(woy, dowmapped, fill = value)) + 
  ...
  scale_fill_gradientn(colours = c("#B5E384", "#FFFFBD", "#FFAE63", "#D61818"), na.value = "white",
                       name = "legendtitle",
